defmodule RestApi.Store do
  ...
  def start_link(_), do: Agent.start_link(fn -> %{users: %{}, posts: %{}, user_id: 0, post_id: 0} end, name: __MODULE__)

  def create_user(data) do
    Agent.get_and_update(__MODULE__, fn state ->
      id = state.user_id + 1
      now = DateTime.utc_now() |> DateTime.to_iso8601()
      user = %User{id: id, name: data["name"], email: data["email"], role: data["role"] || "user", created_at: now, updated_at: now}
  ...
  def list_users(page, per_page, search) do
    Agent.get(__MODULE__, fn state ->
      ...
      items = if search do
        ...
        Enum.filter(items, &(String.contains?(String.downcase(&1.name), q) or String.contains?(String.downcase(&1.email), q)))
      else
        items
      end
  ...
  def delete_user(id) do
    Agent.get_and_update(__MODULE__, fn state ->
      if Map.has_key?(state.users, id) do
        {true, %{state | users: Map.delete(state.users, id)}}
      else
        {false, state}
      end
    end)
  ...
  def create_post(author_id, data) do
    Agent.get_and_update(__MODULE__, fn state ->
      id = state.post_id + 1
      now = DateTime.utc_now() |> DateTime.to_iso8601()
      post = %Post{id: id, author_id: author_id, title: data["title"], body: data["body"], published: data["published"] || false, created_at: now, updated_at: now}
    ...
    Agent.get_and_update(__MODULE__, fn state ->
      case Map.get(state.posts, id) do
        ...
        post ->
          ...
          updated = if Map.has_key?(data, "published"), do: %{updated | published: data["published"]}, else: updated
